...
exports.addProducto = (req, res) => {
  ...
  const {ID_Establecimiento, Nombre, Descripcion, Precio, Disponibilidad, Tipo } = req.body;
  ...
  const checkQuery = 'SELECT * FROM productos WHERE Nombre = ? AND ID_Establecimiento = ?';
  connection.query(checkQuery, [Nombre, ID_Establecimiento], (err, results) => {
    if (err) return res.status(500).send(err);
    if (results.length > 0) {
      return res.status(400).json({ message: 'El nombre del producto ya existe en esta tienda' });
    }

    // Subir la imagen a Cloudinary
    cloudinary.uploader.upload(file.path, { folder: 'products', public_id: `${Nombre}_${ID_Establecimiento}` }, (err, result) => {
      if (err) {
      ...
      const query = 'INSERT INTO productos (ID_Establecimiento, Nombre, Descripcion, Precio, Disponibilidad, Foto, Tipo) VALUES (?, ?, ?, ?, ?, ?, ?)';
      connection.query(query, [ID_Establecimiento, Nombre, Descripcion, Precio, Disponibilidad , imageUrl, Tipo], (err, results) => {
        if (err) return res.status(500).send(err);
        res.status(201).json({ message: 'Producto añadido con éxito', productoID: results.insertId });
      });
    });
  });
};
